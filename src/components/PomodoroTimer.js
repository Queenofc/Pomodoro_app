...
const PomodoroTimer = ({ setStopMusicTrigger }) => {
  ...
  useEffect(() => {
    const unlockAudio = () => {
      // Try playing and immediately pausing to unlock audio playback
      alarmSound.current
        .play()
        .then(() => {
        ...
        .catch((err) => {
          console.error("Audio unlocking failed:", err);
        });
    };
  ...
  useEffect(() => {
    ...
    if (storedTime === 0 && localStorage.getItem("alarmPlaying") === "true") {
      alarmSound.current.loop = true;
      alarmSound.current.play().catch((err) => {
        console.error("Audio play prevented on mount:", err);
      });
    }
  }, []);
  ...
  const applyCustomTime = () => {
    if (isActive) {
      toast.error("Pause the timer before setting a new time!", {
        autoClose: 3000,
      });
      return;
    }
      ...
      <div className={`container ${isDarkMode ? "dark" : "light"}`}>
        ...
        <div className="custom-time">
          ...
          <input
            ...
            onChange={handleCustomTimeChange}
          />
          <button onClick={applyCustomTime} disabled={isActive}>
            Set
          </button>
        </div>
